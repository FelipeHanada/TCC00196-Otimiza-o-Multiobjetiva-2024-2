...
KnapsackSolution Knapsack1FlipBitMovement::move(KnapsackSolution s) {
    s.flip(this->i);
    return s;
}

KnapsackEvaluator::KnapsackEvaluator(int n, int q, std::vector<int> v, std::vector<int> w) {
    this->n = n;
    this->q = q;
    this->v = v;
    this->w = w;
...
int KnapsackEvaluator::evaluate(KnapsackSolution s) {
    int curr_q = this->q;
    int curr_value = 0;
